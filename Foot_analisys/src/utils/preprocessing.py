...
for idx, row in df.iterrows():
    ...
    home_past = df[((df['HomeTeam'] == home) | (df['AwayTeam'] == home)) &
                   (df['Date'] < match_date)].sort_values('Date', ascending=False).head(5)
    if len(home_past) < 5:
        continue
    ...
    home_features = {f'Home_{col}_avg_5': pd.to_numeric(home_past[col], errors='coerce').mean()
                     for col in numeric_cols if col in home_past.columns}

    # Средние фичи последних 5 матчей гостей
    away_features = {f'Away_{col}_avg_5': pd.to_numeric(away_past[col], errors='coerce').mean()
                     for col in numeric_cols if col in away_past.columns}

    # Средние фичи последних 5 личных встреч
    h2h_features = {f'H2H_{col}_avg_5': pd.to_numeric(h2h[col], errors='coerce').mean()
                    for col in ['FTHG', 'FTAG'] if col in h2h.columns}
